# Multi-tenant: many bots, one core
...
Tenants that need bespoke behaviour ship a **handler pack** — a module exporting a
`build(core_router) -> Router`. The gateway resolves `tenant.handler_pack` at
startup and builds one dispatcher per pack, not per tenant.

```python
# packs/furcon/pack.py
from aiogram import Router
from cb_gateway.handlers import build_router as build_core


def build() -> Router:
    router = build_core()  # everything the core provides
    router.include_router(badges)  # this tenant's own commands
    return router
```

Rules that keep this from becoming a fork:

1. **A pack composes the core router; it never replaces it.** Removing a core
   command is `disabled_commands` on the tenant row, not a different pack.
2. **Packs may not import another pack.** Shared behaviour graduates into core.
3. **Packs get the same middleware.** Dedupe, telemetry and the analytics row are
   attached to the dispatcher, not the router, so a pack cannot opt out of
   observability or skip the `message_events` row.
4. **A pack that needs new storage adds a migration** in `cb-api`, distributed on
   `group_id` like everything else. No per-tenant schema.
5. **Custom commands are registered in `command_catalog`** with their tenant, so
   `/commands` stays accurate per tenant.
