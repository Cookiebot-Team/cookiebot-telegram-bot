...
description: "The real end-to-end suite: gateway and sandbox as two processes over HTTP"
---

`docs/SANDBOX.md` describes a workbench a person drives by hand. `CB_QA_SANDBOX=1`
(`qa/sandbox_harness.py`) drives the *acceptance* suite through `tg_sandbox`'s
store, but it feeds updates straight into the aiogram dispatcher in-process and
only mirrors them into the sandbox — `tg_sandbox.control_api` is never called
and `getUpdates` polling never runs. Neither one drives the gateway through
HTTP end to end.

`qa/e2e/` closes that gap. It starts the real, unmodified `tg_sandbox.app:app`
...
|---|---|---|---|
| Telegram fake | `qa/mock_telegram.py`, in-process | `tg_sandbox`, in-process | `tg_sandbox`, a real subprocess on a real port |
...
1. **Two subprocesses.** `tg_sandbox.app:app` on a free port with its own
...
```bash
TG_SANDBOX_DB=sandbox-e2e.duckdb python scripts/cb.py sandbox   # :8083
...
Picked from what the [progress board](/docs/progress) marks done and green, one file
...
- Anything not yet ported (the progress board's "planned"/"not
...
`tg_sandbox` — recorded here because each has a matching regression test and
...
1. **`join_chat` did not persist the join's own service message.**
   ...
   reply_markup=keyboard)` — and `tg_sandbox` validates reply targets for
...
`by_user_id` — a shape real Telegram cannot produce, but `tg_sandbox` allows.
