...
`cb-gateway` hands work to `cb-worker` through `cb_gateway.queue.enqueue(job,
*args, **kwargs) -> bool` — one lazily created `arq` pool on the same
...
- Mojo's `String.lower()` is full Unicode case folding and costs **445 ns/op**, ~100× everything else in the parser. An ASCII fast path plus an allocation-free case-insensitive compare took the native parse from 824 to 28.5 ns/op. Any port doing string work will hit this.
- The `.so` built by Mojo imports and runs under Python 3.14, so the interpreter version is not the blocker here. The call overhead is.
