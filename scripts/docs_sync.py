...
#: Generated by `cb.py sandbox-config` from `cb_core.textmatch.COMMAND_ALIASES`
#: and `scripts/spec.py`. Reading it here means the user-facing command
#: reference is the parser's own alias table rather than a hand-kept list — the
#: one table on the site a reader is most likely to act on, and the one that
#: goes stale first.
...
def commands() -> list[dict[str, object]]:
    """Every command the parser accepts, with its aliases and the feature it
    belongs to. Empty (rather than an error) when the sandbox config has not
    been generated: a missing user-facing table is a worse failure than a
    docs-sync that refuses to run."""
    if not SANDBOX_CONFIG.is_file():
        return []
    raw = json.loads(SANDBOX_CONFIG.read_text(encoding="utf-8"))
    by_id = {f.id: f for f in FEATURES}
    out: list[dict[str, object]] = []
    for command in raw.get("commands", []):
        feature = by_id.get(command.get("feature_id") or "")
        out.append(
            {
                "primary": command["primary"],
                "aliases": command.get("aliases") or [],
                "feature_id": command.get("feature_id"),
                "area": feature.area if feature else "other",
                "title": command.get("title") or (feature.title if feature else ""),
                "hint": command.get("hint") or "",
                "status": command.get("status") or "unknown",
                "url": f"/docs/features/{feature.id}" if feature else None,
            }
        )
    return sorted(out, key=lambda c: str(c["primary"]))
